...
const Cart = () => {
  ...
  const { setIsQRCodeModalOpen, setQrCodeData } = useQRCode();
  ...
  const openModaleQRcode1 = async (event) => {
    event.preventDefault();
    ...
    // Correctly declare and assign qrData after the async call
    const qrData = await handleCheckOut(); // Generate QR Code
    ...
    console.log("Setting QR Code Data in Context:", qrData); // ✅ Debugging
    setQrCodeData(qrData); // ✅ Store QR Code globally
    setIsQRCodeModalOpen(true); // ✅ Open the modal
  };
