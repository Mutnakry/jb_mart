...
function Supplier() {
  ...
  const getCategoryName = (id) => {
    const category = categorys.find(cat => String(cat.id) === String(id));
    return category ? category.cat_names : "N/A";
  };

  const getUnitName = (id) => {
    const unit = Units.find(unit => String(unit.id) === String(id));
    return unit ? unit.names : "N/A";
  };

  const getBrandName = (id) => {
    const brand = Brands.find(brand => String(brand.id) === String(id));
    return brand ? brand.brand_names : "N/A";
  };
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const matchesBrands = selectedBrands === "ទាំងអស់" || String(order.brand_id) === String(selectedBrands);
    const matchesCategories = selectedCategorys === "ទាំងអស់" || String(order.category_id) === String(selectedCategorys);
    ...
    const remainingAmount = totalAmount - balanceAmount;
    ...
    return matchesCurrency && matchesCustomer && matchesDate && matchesPaymentStatus && matchesBrands && matchesCategories && matchesUnits;
  });


  const HandClear = () => {
    setSelectedBrands('ទាំងអស់');
    setSelectedCategorys('ទាំងអស់');
    setSelectedCustomer('ទាំងអស់');
    setFilterAmountDi('ទាំងអស់');
    setFilterDate('ទាំងអស់');
    setSelectedCategorys('ទាំងអស់');
    setSelectedCustomer('ទាំងអស់');
    setSelectedUnits('ទាំងអស់');
    setSelectedCurrency('ទាំងអស់');
  }
  ...
  const exportToExcel = () => {
    const wsData = filteredOrders.map((order, index) => ({
      ...
      'ប្រភេទទំនិញ': `${order.cat_names} `,
      'ម៉ាលយីយោ': `${order.brand_names} `,
  ...
  return (
    <div>
      ...
      <div className='py-12 print:py-0 print:px-0 px-1 sm:ml-64 print:ml-0 md:w-auto w-[860px] bg-gray-200 dark:bg-gray-950'>
        <div className="w-full p-4 mt-10 bg-white dark:border-gray-700 animate-fade-up animate-duration-2000 animate-ease-in-out">
          ...
          <div className='grid md:grid-cols-4 grid-cols-1 gap-3 border-t-2 border-blue-500 bg-gray-200/50 py-6 px-2'>
            ...
            <div className="space-y-2 ">
              ...
              <select className='input_text block' value={selectedCurrency} onChange={(e) => setSelectedCurrency(e.target.value)}>
                <option value="ទាំងអស់">ទាំងអស់</option>
            ...
            <div className="space-y-2">
              <label className='text-lg font-NotoSansKhmer' htmlFor="">ពេលទិញ:</label>
              <select
                className='input_text font-NotoSansKhmer text-md block'
            ...
            <div className="space-y-2 items-center flex justify-center">
              <button onClick={HandClear} className='bg-blue-500 p-2 rounded-full text-white'>
                <IoIosRefresh className='hover:rotate-45 duration-300' />
              </button>
            </div>
          </div>
            ...
            <div className="px-6 pt-6 print:border-0 print:shadow-none hidden  print:block">
              ...
              <div className='flex justify-between py-6'>
                ...
                <div className='space-y-1'>
                  ...
                  <p className='uppercase'>កាលបរិច្ខេទ : <span className='font-NotoSansKhmer'>{filterDate}</span></p>
                  <p className='uppercase'>រូបិយប័ណ្ណ : {selectedCurrency}
                  </p>
                </div>
                <div className='space-y-2'>

                  <p>ឯកតា : {selectedUnits === "ទាំងអស់" ? "ទាំងអស់" : getUnitName(selectedUnits)}</p>
                  <p>ម៉ាកយីហោ : <span className="font-NotoSansKhmer">{selectedBrands === "ទាំងអស់" ? "ទាំងអស់" : getBrandName(selectedBrands)}</span></p>
                  <p>ប្រភេទទំនេញ : {selectedCategorys === "ទាំងអស់" ? "ទាំងអស់" : getCategoryName(selectedCategorys)}</p>

                </div>
                <div className='space-y-2'>
                  <p>ពេលទិញ : {filterAmountDi}
                  </p>
                </div>
                ...
                <table className="min-w-full table-auto">
                  <thead className="text-white bg-blue-600/95">
                    <tr className="font-bold font-NotoSansKhmer">
                      ...
                      <th className="px-1 py-2">កាលបរិច្ឆេទ</th>
                      <th className="px-1 py-2">អតិជន</th>
                      <th className="px-1 py-2 whitespace-nowrap">ឈ្មោះផលិតផល</th>
                      <th className="px-1 py-2 ">ប្រភេទទំនិញ</th>
                      <th className="px-1 py-2 ">ម៉ាលយីយោ</th>
                      <th className="px-1 py-2">ចំនួន</th>
                      <th className="px-1 py-2">ចំនួនទឺកប្រាក់សរុប</th>
                      <th className="px-1 py-2">ទឺកប្រាក់ដែលបានបង់</th>
                      <th className="px-1 py-2">នៅនៅខ្វះទឺកប្រាក់</th>
                      <th className="px-1 py-2">បញ្ចុះតម្លៃ</th>
                    </tr>
                  </thead>
                  {loading ? (
                    <p>Loading...</p>
                      ...
                      {filteredOrders.map((order, index) => {
                        ...
                        return (
                          <motion.tr
                            ...
                            <td>{index + 1}</td>
                            <td className="px-1 py-2 whitespace-nowrap">{formatDateToKhmer(new Date(order.date_order))}</td>
                            <td className="px-1 py-2 whitespace-nowrap">{order.full_names || order.business_names}</td>
                            <td className="px-1 py-1 ">
                              {order.pro_names}
                            </td>
                            <td className="px-1 py-1 ">{order.cat_names || 'N/A'}</td>
                            <td className="px-1 py-1">{order.brand_names || 'N/A'}</td>
                  ...
                  <tfoot>
                    <tr className="font-bold bg-gray-200">
                      <td colSpan='6' className="text-right px-1 py-2">សរុប:</td>
                      <td className="px-1 py-2">{totalQTY} ផលិតផល</td>
                      ...
                      <td className="px-1 py-2">{totalDiscountSum} $</td>
                    </tr>
                  </tfoot>
                </table>
              </AnimatePresence>
