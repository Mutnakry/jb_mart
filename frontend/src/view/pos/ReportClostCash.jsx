...
const Cost = () => {
    ...
    const GetAllData = async () => {
        try {
            const resClostReport = await axios.get(`${API_URL}/api/opencash/clostreport`);
            const resCost = await axios.get(`${API_URL}/api/opencash/cost`);
            const resProduct = await axios.get(`${API_URL}/api/opencash/product`);
            const resSumProduct = await axios.get(`${API_URL}/api/opencash/sumProduct`);
            const resSumCuaromerReturn = await axios.get(`${API_URL}/api/opencash/customer_return`);
            // const resClostReport = await axios.get(`${API_URL}/api/opencash/clostreport`);
            ...
            if (resClostReport.data.length > 0) {
                ...
                setClost_date(reportData.end_date); // ✅ Use `end_date` instead of `clost_date`
            } else {
                ...
                <div className="w-full p-4 print:p-0 mt-10  print:mt-0 bg-white dark:border-gray-700 animate-fade-up animate-duration-2000 animate-ease-in-out">
                    ...
                    <div id='invoiceReport'>
                        ...
                        <div className='bg-green-200 text-gray-700 py-2 px-6 border-t border-blue-500 mt-4'>
                            ...
                            <br />
                            ចំណេញសរុប​ ($) : ${((totalAmountPrice) - (totalAmountPriceCost + TotalAmoountOpenBalance))}
                        </div>
                    </div>
                    <div >
